 ...
 */
public class JedisClientPool_231 {
    /**
     * 属性文件加载对象
     */
    //private static PropertiesLoader loader = PropertiesLoader.getIntances("hys");

    public static JedisClientPool_231 jedisClientPool231 = getInstance();
    public static JedisPool jedisPool;
    public static int maxTotal;
    public static int maxIdle;
    public static int maxWaitMillis;

    public static synchronized JedisClientPool_231 getInstance() {
        if (null == jedisClientPool231) {
            jedisClientPool231 = new JedisClientPool_231();
        }
        return jedisClientPool231;
    }

    public JedisClientPool_231() {
        if (null == jedisPool) {
            init();
        }
    }
    ...
    public static void init() {
        ...
        String host =  properties.getProperty("redis.host.231");// "localhost";
        int port = Integer.parseInt(properties.getProperty("redis.port.231"));// 6379;
        int timeout = Integer.parseInt( properties.getProperty("redis.timeout"));// 60000;
        String pwd = StringUtils.isBlank(properties.getProperty("redis.password.231"))?"":properties.getProperty("redis.password.231");
        // 构造连接池
